                ...
                <?php foreach ($data->Manifest as $manifestGroup) {
                    $languageItem = $manifestGroup->Get('ContentLang',1);
                    
                ?>
                    <li class="list-group-item">
                        <h4><?php $languageItem('Title');?></h4>
                        <ul class="media-list">
                        <?php 
                        
                        foreach ($languageItem->Positions as $position) {
                                $positionLangageItem    = $position->Get('ContentLang',1);
                                $character              = $positionLangageItem->Get('Character','ContentLang',1);
                                $canEditCharacter       = $positionLangageItem->Get('Character','ContentLang',1,'CanEdit');
                                $canApply               = $positionLangageItem->Get('CanApply');
                                Debug($character);
                        ?>
                            <li class="media">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            ...
                                            <?php
                                                        ...
                                                        if($position->Equals('Status','Open') && $canApply)
                                                        {
                                                    ?>
