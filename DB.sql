...
SHOW WARNINGS;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `loggedoutgroups` AS select `usergroups`.`Id` AS `Id`,`usergroups`.`Name` AS `name`,`usergroups`.`Created` AS `created` from `usergroups` where (`usergroups`.`AssignLoggedOutUsers` = 1);
SHOW WARNINGS;
...
SHOW WARNINGS;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_userdata` AS select `userdata`.`Users_id` AS `UserId`,`userdatatypes`.`Name` AS `Name`,`userdatatypes`.`Type` AS `Type`,`userdata`.`Value` AS `Value` from (`userdata` left join `userdatatypes` on((`userdata`.`UserDataTypes_id` = `userdatatypes`.`Id`)));
SHOW WARNINGS;
...
SHOW WARNINGS;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_usersgroups` AS select `usergroups`.`Id` AS `Id`,`usergroups`.`Name` AS `Name`,`usergroups`.`Created` AS `Created`,`usergroups`.`AssignLoggedOutUsers` AS `AssignLoggedOutUsers`,`usersingroups`.`Users_id` AS `UserId` from (`usergroups` left join `usersingroups` on((`usergroups`.`Id` = `usersingroups`.`UserGroups_id`)));
SHOW WARNINGS;
